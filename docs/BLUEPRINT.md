...
## 4. Рубрика скоринга v2: «какой адрес сейчас» × «что происходит» (согласовано 2026-09-09)

Регистрация в ОАЭ сама по себе не признак. Признак — пара: **тип текущего адреса** и
**событие**, которое делает этот адрес недостаточным. Пороги — ВЫБРАНО из ресерча 01,
калибруются по eval в день 1.

### Ось A. Тип текущего адреса (выводится моделью, с уверенностью)
| Код | Что это | По каким признакам выводим |
|---|---|---|
| A0 `none` | В ОАЭ не зарегистрирована | нет лицензии в реестрах, нет адреса в ОАЭ на сайте/картах |
| A1 `flexi` | Free-zone flexi-desk / виртуальный адрес | адрес = адрес фризоны или бизнес-центра, нет своей точки на Google Maps, на сайте только почтовый адрес |
| A2 `serviced` | Serviced-офис в бизнес-центре (в т.ч. у конкурентов) | адрес = известный БЦ (Regus, Smart Hub и др.) |
| A3 `own` | Собственный офис с Ejari | адрес в обычном бизнес-здании, есть точка на картах, вакансии указывают этот адрес |

### Ось B. Событие-триггер (из новостей, вакансий, сайта; с evidence_url)
| Код | Событие | Почему создаёт потребность |
|---|---|---|
| B1 `entering_uae` | Объявила выход на рынок ОАЭ / открытие представительства | нужен полный пакет: регистрация + офис + визы + счёт |
| B2 `hiring` | ≥ 3 открытых вакансии в Дубае / анонс роста команды | визы привязаны к метражу (~1 виза на 9 кв. м по Ejari); flexi даёт 1–3 визы |
| B3 `renewal` | Лицензии ≥ 10 мес (продление через 1–2 мес) | продление mainland требует действующий Ejari; кросс-селл бухгалтерия/налог |
| B4 `relocating` | Переезд команды / HQ в Дубай | офис + семейные визы + релокация |
| B5 `funding` | Раунд / крупный контракт в регионе | бюджет появился, штат вырастет |
| B0 `none` | Сигналов нет | — |
...
\* A3 + hiring → MEDIUM только если рост ≥ 5 вакансий (офис перерастают); иначе LOW.

### Модификаторы (после матрицы)
- Язык компании RU (сайт/вакансии/новости на русском) → +1 ступень, максимум HIGH: ЦА SORP.
- Сфера с потребностью в складе/производстве → −1 ступень: не профиль Barsha Heights.
- Размер < 2 человек и нет B1/B4 → LOW независимо от остального.
- Уверенность модели в оси A < 0.5 → tier не повышается выше MEDIUM, в карточке флаг «адрес не подтверждён».

### Детерминированные инварианты (проверяются кодом, не моделью; Т1-мутируемые)
1. A3 `own` + B0 `none` → всегда LOW.
2. A0 `none` без B1/B4 и без сигналов интереса к ОАЭ → LOW.
3. HIGH требует ≥ 1 `evidence_url` для оси B.
4. Tier не выше MEDIUM при уверенности в оси A < 0.5.

Нарушение инварианта → результат помечается `invalid`, попадает в отчёт как `нарушений M`
и в ручную очередь, а не в таблицу лидов.

### Что для модели в промпте, что для кода
- Модель: вывести A и B с уверенностью, причинами и evidence_url; предложить tier.
- Код: пересчитать tier по матрице из A/B, сравнить с предложенным моделью, при расхождении
  взять матричный и записать расхождение в лог (Е2: верим свидетельству, не вердикту).
