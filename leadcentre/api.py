...
async def _budget_handler(_: Request, exc: ProviderBudgetError) -> JSONResponse:
    ...
    503 и внятный текст: чинится кошельком, повторять запрос бессмысленно. 500 со
    стектрейсом заставил бы клиента искать баг в коде, которого там нет.
    """
    return _error(
        503, "provider_budget", str(exc),
