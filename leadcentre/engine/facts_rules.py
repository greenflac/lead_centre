...
TYPE_MARKERS: dict[RequestType, tuple[str, ...]] = {
    ...
    RequestType.RENEWAL: (
        ...
        # которых не было и из-за которых «продлеваем лицензию» уезжало в регистрацию
        # (ИЗМЕРЕНО 2026-09-09). «renew» покрывает renewal/renewing/renewals.
        # «истёк» пишут и через ё, и через е — обе формы, иначе половина обращений мимо.
        "продлен", "продли", "продлева", "renew", "истекает", "истекл", "истёк", "истек",
        "заканчивается", "expires", "expiry", "expiring", "ежегодн", "annual fee",
    ),
    RequestType.BANK: (
        # Счёт открывают, не называя банк: «открыть счёт», «open an account» — сплошь и рядом.
