...
Три исхода вместо двух (Р1):
  ...
  * `MACHINE` — наша машинная транслитерация (или её же значение из кэша);
...
из реестра и наша догадка — разные вещи (Е2), и разница обязана дожить до карточки, а не
потеряться в первой же переменной `name`. Поэтому у результата есть `is_official`,
...
TRANSLIT_MODEL = ANTHROPIC_MODEL          # ВЫБРАНО: та же дешёвая модель, что для коротких
MAX_TOKENS = 200                          # ВЫБРАНО: ответ — одна строка названия
MAX_NAME_CHARS = 300                      # ВЫБРАНО: длиннее — это не название, а мусор
...
class TranslitStatus(str, Enum):
    ...
    MACHINE = "machine"           # наша транслитерация
...
def source_latin_name(entity: dict) -> tuple[str, str] | None:
    ...
    Смотрим и `otherNames`, и `transliteratedOtherNames`: во втором лежит ASCII-написание
    из реестра, и оно всё равно ближе к истине, чем наша догадка.
    """
...
def cache_key(name: str) -> str:
    """Ключ — хэш нормализованного названия плюс версия промпта: сменив промпт, мы получим
    другие ответы, и старые не должны выдаваться за новые (Е2)."""
...
def _junk_reason(name: str) -> str | None:
    """Мусор отсеиваем до сети (П2): пустое, слишком длинное, без арабских букв."""
    cleaned = name.strip()
    if not cleaned:
...
def warm_cache(names: list[str]) -> tuple[int, int, list[TranslitName]]:
    """Прогреть кэш списком названий. Числами, а не флагом (Е3): сколько получили,
