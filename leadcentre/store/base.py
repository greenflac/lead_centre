...
их пишет отрисовка (Е1), а читатель, которому нужен английский, берёт коды. Старая строка
без кодов — это отдельный исход `no_codes` («не смогли восстановить причины», Р1), а не
молчаливая подмена английского русским: ровно этот дефект и ловится.
"""
...
class ScoreRow:
    ...
    источника текста, которые разъедутся (Е1, тем же машинным способом, что и в `Score`).
...
class LeadCard:
    ...
    def reasons(self) -> RestoredReasons:
        """Причины карточки на всех языках либо исход «не смогли» (Р1).

        Живёт здесь, а не в `api.py`: карточку поднимают и CLI, и отчёт, и HTTP —
        способ восстановить причины должен быть один (Е1).
        """
