...
любой текст и получает карточку. Измеренный прогон 2026-09-11 — HIGH, четыре причины, срок
20 дней (посчитан от даты обращения), уверенность 0,90, черновик по-русски, 5,4 секунды,
запись в Supabase. Команда `make shots-live` поднимает живой контур и снимает эти экраны
сама; ей нужен оплаченный ключ модели, поэтому готовых кадров живого режима в репозитории
нет — вместо снимка, который нельзя воспроизвести, здесь команда, которая его делает.
