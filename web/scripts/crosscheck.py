...
одно и то же — дефект (Е1), и единственная защита от расхождения — регулярная сверка
...
Негативный контроль прибора (И5): в набор добавлены две подложные строки — в одной
подменён приоритет, в другой текст черновика; прибор обязан их назвать расхождениями.
Без этого «0 расхождений» не отличить от «прибор ничего не сравнивает».

Три исхода (Р1): СОШЛОСЬ / РАСХОЖДЕНИЕ / НЕ СМОГЛИ СВЕРИТЬ (node или tsc недоступны).
