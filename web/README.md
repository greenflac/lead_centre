...
| `08-live-backend.png`, `09-live-new-request.png` | Live mode. Shot by `make shots-live`, which brings up the API without `OFFLINE`, builds the dashboard against it and refuses to shoot unless `/health` reports `offline: false`. Needs a funded model key. |
