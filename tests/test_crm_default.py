...
Проверяется четыре утверждения, и каждое — отдельная мутация:
  ...
  2. заглушка возвращает исход «пропущено», а не «отправлено» (Е2: вердикт — из того,
...
Ожидаемое — литералы (Т2): имена приёмников и исходы записаны строками, из
`leadcentre.crm` не импортируются ни `SINKS`, ни `SKIPPED`, ни `SENT`.
Сети нет (Т4): ни один тест не вызывает `send` у HubSpot с настоящим токеном.
"""
