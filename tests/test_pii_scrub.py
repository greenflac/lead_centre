...
Что здесь проверяется и почему именно так:
  ...
  * порог `MIN_PHONE_DIGITS` — с обоих краёв (8 цифр не номер, 9 цифр номер), чтобы
    сдвиг порога в любую сторону красил тест, а не только в одну (Т1, Т3);
...
Ожидаемое — литералы (Т2): ни `MIN_PHONE_DIGITS`, ни `PHONE_MASK`, ни `EMAIL_MASK`
из `extract` не импортируются, иначе тест поедет вместе с константой и промолчит.
Сети нет нигде (Т4): OFFLINE, подставные провайдеры, подменённый `urlopen`.
"""
...
def assert_no_pii(payload: str) -> None:
    """Негативный контроль тела запроса: ПД нет, а маски на месте (И5).
...
def test_several_contacts_in_one_text_are_all_masked_and_counted_by_numbers():
    """Счётчики числами, а не флагом (Е3): «что-то вырезали» не отличает 1 от 3."""
    result = scrub_pii(
...
def test_text_without_contacts_is_returned_byte_for_byte(text):
    """Негативный контроль (И5): вход, на котором прибор обязан молчать."""
    result = scrub_pii(text)
...
def test_phone_threshold_is_nine_digits_from_both_sides(text, expected, phones):
    ...
    Сдвиг `MIN_PHONE_DIGITS` в любую сторону красит этот тест (Т1): вниз — покраснеет
